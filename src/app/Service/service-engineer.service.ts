...
export class ServiceEngineerService {
  ...
  resolveTicket(user : Login) {
    return this.http.post("api/serviceEngineer/resolveTicket", user, {responseType:'text'});
  } 

  changePriority(ticket : TicketDetails) {
    return this.http.post("api/serviceEngineer/updatePriority", ticket, {responseType : 'text'})
  }
